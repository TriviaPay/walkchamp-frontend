...
export async function fetchLiveUnlimitedChallenges(opts?: {
  ...
  try {
    const [dedicatedLive, dedicatedFinished] = await Promise.all([
      Promise.all(PRIMARY_LIVE.map((p) => fetchPathRows(p))),
      Promise.all(PRIMARY_FINISHED.map((p) => fetchPathRows(p))),
    ]);
    liveBatches = dedicatedLive;
    finishedBatches = dedicatedFinished;

    const primaryLiveCount = dedicatedLive.reduce((n, b) => n + b.length, 0);
    const primaryFinishedCount = dedicatedFinished.reduce((n, b) => n + b.length, 0);

    // Fallback query-param paths only when primary returned nothing usable.
    if (primaryLiveCount === 0) {
      const fallbackLive = await Promise.all(FALLBACK_LIVE.map((p) => fetchPathRows(p)));
      liveBatches = liveBatches.concat(fallbackLive);
    }
    if (primaryFinishedCount === 0) {
      const fallbackFinished = await Promise.all(FALLBACK_FINISHED.map((p) => fetchPathRows(p)));
      finishedBatches = finishedBatches.concat(fallbackFinished);
    }

    // Known-id + races/my-active only when still empty after dedicated paths.
    const afterFallbackLive = liveBatches.reduce((n, b) => n + b.length, 0);
    if (afterFallbackLive === 0) {
      const [known, racesMine] = await Promise.all([
