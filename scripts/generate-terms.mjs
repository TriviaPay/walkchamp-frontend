/**
 * Generates constants/termsAndConditions.ts
 * Run: node scripts/generate-terms.mjs
 *
 * DRAFT – LEGAL REVIEW REQUIRED BEFORE PUBLICATION
 * (Internal only — do not surface this notice in the user-facing UI.)
 */
import fs from "node:fs";
import path from "node:path";

/** Convert markdown-style "- " bullets to "• " for the shared renderer. */
...
const out = [];
out.push(`/**`);
out.push(` * Walk Champ Terms and Conditions — structured content.`);
out.push(` *`);
out.push(` * DRAFT – LEGAL REVIEW REQUIRED BEFORE PUBLICATION`);
out.push(` * This notice is for internal engineering/legal use only.`);
out.push(` * Do not render it in the user-facing UI after legal approval.`);
out.push(` *`);
out.push(` * CIN: insert the verified Corporate Identification Number before publication.`);
out.push(` * Do not invent or display a placeholder CIN to end users.`);
out.push(` */`);
out.push(``);
out.push(`export type TermsSection = {`);
out.push(`  id: string;`);
out.push(`  number: number;`);
out.push(`  title: string;`);
out.push(`  body: string;`);
out.push(`  keywords: string[];`);
out.push(`};`);
out.push(``);
out.push(`export type TermsDocument = {`);
out.push(`  version: string;`);
out.push(`  effectiveDate: string;`);
out.push(`  title: string;`);
out.push(`  lastUpdatedLabel: string;`);
out.push(`  intro: string;`);
out.push(`  supportEmail: string;`);
out.push(`  publicUrl: string;`);
out.push(`  sections: TermsSection[];`);
out.push(`};`);
out.push(``);
out.push(`export const TERMS_SUPPORT_EMAIL = "admin@miragaming.com";`);
out.push(`export const TERMS_PUBLIC_URL = "https://walkchamp.app/terms";`);
out.push(``);
out.push(`export const TERMS_DOCUMENT: TermsDocument = {`);
out.push(`  version: "1.0",`);
out.push(`  effectiveDate: "2026-07-21",`);
out.push(`  title: "Walk Champ Terms and Conditions",`);
out.push(`  lastUpdatedLabel: "July 21, 2026",`);
out.push(`  intro: ${JSON.stringify(INTRO)},`);
out.push(`  supportEmail: TERMS_SUPPORT_EMAIL,`);
out.push(`  publicUrl: TERMS_PUBLIC_URL,`);
out.push(`  sections: [`);
...
for (const s of sections) {
  ...
  out.push(`      keywords: ${JSON.stringify(s.keywords)},`);
  out.push(`      body: \`${escape(normalize(s.body))}\`,`);
  out.push(`    },`);
}
...
out.push(`};`);
out.push(``);
out.push(`export function searchTermsSections(query: string): TermsSection[] {`);
out.push(`  const q = query.trim().toLowerCase();`);
out.push(`  if (!q) return TERMS_DOCUMENT.sections;`);
out.push(`  return TERMS_DOCUMENT.sections.filter((section) => {`);
out.push(`    const haystack = [\`\${section.number}\`, section.title, section.body, ...section.keywords]`);
out.push(`      .join(" ")`);
...
const target = path.join(process.cwd(), "constants", "termsAndConditions.ts");
