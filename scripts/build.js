...
function getDeploymentDomain() {
  if (process.env.EXPO_PUBLIC_WEB_URL) {
    return stripProtocol(process.env.EXPO_PUBLIC_WEB_URL);
  }
  ...
  console.error(
    "ERROR: No deployment domain found. Set EXPO_PUBLIC_WEB_URL or EXPO_PUBLIC_APP_URL",
  );
  process.exit(1);
}
...
async function startMetro(expoPublicDomain) {
  ...
  const env = {
    ...process.env,
    EXPO_PUBLIC_WEB_URL: `https://${expoPublicDomain}`,
  };
