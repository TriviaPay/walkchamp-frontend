...
import { ChannelRefCounter } from "@/services/channelRefCount";
...
export function subscribeToChannel(channelName: string): ChannelAdapter | null {
  const ch = _channelRefs.acquire(channelName, () => {
    const client = getClient();
    if (!client) return null;
    return new ChannelAdapter(client.subscribe(channelName));
  });
  if (__DEV__) {
    const hint = channelName.includes("private-user")
      ? "private-user"
      : channelName.length > 24
        ? `${channelName.slice(0, 24)}…`
        : channelName;
    // Lazy import avoids circular deps with utils that may pull services.
    void import("@/utils/perfLogger").then(({ perf }) => {
      perf.channelRefCount(hint, _channelRefs.count(channelName));
    }).catch(() => {});
  }
  return ch;
}
