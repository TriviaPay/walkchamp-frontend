 ...
 */
export async function runCoalescedAuthed<T>(
  key: string,
  userId: string | null | undefined,
  fn: () => Promise<T>,
): Promise<T | undefined> {
  const { captureAuthSessionGuard } = require(
    "@/services/authSessionGeneration",
  ) as typeof import("@/services/authSessionGeneration");
  const guard = captureAuthSessionGuard(userId);
  if (!guard) return undefined;
  const result = await runCoalesced(key, fn);
  if (!guard.stillValid()) return undefined;
  return result;
}
